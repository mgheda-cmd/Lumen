...
(function() {
    ...
    // Mini-HUD visuel en haut à droite sur MEXC
    function createHud() {
        if (document.getElementById('lumen-web-hud')) return;
        const hud = document.createElement('div');
        hud.id = 'lumen-web-hud';
        hud.style.cssText = 'position:fixed;top:60px;right:20px;z-index:999999;background:rgba(15,23,42,0.95);border:1.5px solid #00E5FF;border-radius:10px;padding:8px 12px;color:#fff;font-family:system-ui,sans-serif;font-size:11.5px;box-shadow:0 10px 30px rgba(0,0,0,0.8);backdrop-filter:blur(8px);display:flex;align-items:center;gap:8px;pointer-events:none;transition:all 0.3s';
        hud.innerHTML = '🟢 <span style="font-weight:800;color:#00E5FF">Lumen Web Trader Actif</span> <span style="font-size:9.5px;color:#94A3B8;background:rgba(0,229,255,0.15);padding:2px 6px;border-radius:4px">Frais 0.02%</span>';
    ...
    function notifyHud(msg, color='#00E5FF') {
        ...
        hud.innerHTML = `⚡ <span style="font-weight:800;color:${color}">${msg}</span>`;
        setTimeout(() => {
            if (hud) {
    ...
    async function executeMarketOrder(signal) {
        try {
            ...
            const buttons = Array.from(document.querySelectorAll('button, div[role="tab"], span'));
            ...
            const targetBtn = actionButtons.find(b => {
                ...
                if (isBuy) {
                    return txt.includes('open long') || txt.includes('ouvrir long') || txt.includes('buy / long') || txt.includes('acheter');
                } else {
                    return txt.includes('open short') || txt.includes('ouvrir short') || txt.includes('sell / short') || txt.includes('vendre');
                }
            });
            ...
            if (targetBtn && !targetBtn.disabled) {
                ...
                console.log('[Lumen Web Trader] Ordre cliqué avec succès !');
            } else {
    ...
    console.log('[Lumen Web Trader] En écoute des signaux de Lumen Charts...');
})();
